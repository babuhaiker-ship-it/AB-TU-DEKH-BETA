...
from fastapi.responses import RedirectResponse, JSONResponse, StreamingResponse # <-- MODIFIED
...
# --- Pyrogram Imports (at the top with other pyrogram imports) ---
from pyrogram.file_id import FileId, FileType # <-- ADDED
from pyrogram.raw.functions.upload import GetFile # <-- ADDED
from pyrogram.raw.types import InputFileLocation, InputDocumentFileLocation, InputPhotoFileLocation # <-- ADDED
...
class BotConfig:
    ...
    MINI_APP_URL = os.environ.get("STREAM_BASE_URL", "https://ab-tu-dekh-beta-1-u3lq.onrender.com") # Frontend and Backend are on the same domain # MODIFIED

    # --- ADDED THIS ---
    # This MUST be the public URL of your Render Web Service.
    # e.g., "https://your-app-name.onrender.com"
    STREAM_BASE_URL = os.environ.get("STREAM_BASE_URL", "https://ab-tu-dekh-beta-1-u3lq.onrender.com")
...
# --- ADDED THESE NEW UTILITY FUNCTIONS ---

def decode_file_id(file_id: str):
    """Decodes a Pyrogram file_id into a raw InputFileLocation object."""
...
def get_mime_type(file_name: str = None, file_type: FileType = None) -> str:
    """Determines the correct MIME type for the Content-Type header."""
    if file_name:
...
async def get_api_config(auth: dict = Depends(verify_telegram_init_data)):
    ...
    return {
        "buy_bot_url": config.BUY_BOT_URL,
        "bot_username": config.BOT_USERNAME.lstrip('@'),
        "default_category": "Action" # You can make this dynamic if you want
    }
...
async def get_api_profile(auth: dict = Depends(verify_telegram_init_data)):
    ...
    ref_link = f"https://t.me/{config.BOT_USERNAME.lstrip('@')}?start=ref_{user_id}" # <-- ADDED
...
async def get_api_stream_url(video_uuid: str, auth: dict = Depends(verify_telegram_init_data)):
    """
    MODIFIED: Returns a temporary, secure URL to the bot's own streaming endpoint.
    """
    user_id = auth["user_id"]
    ...
    # Generate a temporary token for the stream URL to prevent link abuse
...
async def stream_video(video_uuid: str, request: Request, token: str = None, download: bool = False):
    """
    Streams a video file directly from Telegram, acting as a proxy.
    This supports HTTP Range requests for video seeking.
    """
    if not token:
        raise HTTPException(status_code=401, detail="Unauthorized: Missing stream token")
    ...
    try:
        ...
        if get_current_time() - int(token_timestamp_str) > 3600: # 1-hour link expiry
            raise HTTPException(status_code=401, detail="Unauthorized: Expired stream token")
        if token_uuid != video_uuid:
            raise HTTPException(status_code=403, detail="Forbidden: Token mismatch")
        
        if not user_has_token(int(token_user_id_str)) and not is_premium_user(int(token_user_id_str)):
            raise HTTPException(status_code=403, detail="Forbidden: User access has expired")

    except Exception:
        raise HTTPException(status_code=401, detail="Unauthorized: Invalid stream token")
    ...
    if not input_file_location:
        raise HTTPException(status_code=500, detail="Could not retrieve file information")
    ...
    headers = {
        ...
        "Content-Disposition": f"{'attachment' if download else 'inline'}; filename=\"{file_name or video_uuid}\""
    }
        ...
        while bytes_to_send > 0:
            try:
                chunk = await app.invoke(
                    GetFile(location=input_file_location, offset=current_offset, limit=min(CHUNK_SIZE, bytes_to_send))
                )
                ...
                logger.error(f"Error during streaming chunk for {video_uuid}: {e}")
                break
    ...
    try:
        ...
        if deep_link_arg:
            ...
            try:
                ...
                # --- ADDED THIS NEW CASE ---
                elif deep_link_arg == 'refresh_request':
                    deep_link_type = 'refresh_request'
