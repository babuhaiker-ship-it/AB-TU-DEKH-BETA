...
async def send_and_replace_message(client: Client, chat_id: int, message_id_to_edit_or_delete: int, new_message_type: str, video_data: dict = None, reply_markup: InlineKeyboardMarkup = None, text_content: str = None, force_new_message: bool = False) -> tuple[bool, Message | str]:
    """
    Sends or updates a message with a clear, robust, multi-step logic for handling videos.
    Flow: Tries to edit -> on failure, falls back to sending a new message.
    Fallback Flow: Deletes old message -> tries copy_message -> on failure, tries send_video.
    Handles temporary (FileReferenceExpired) and permanent errors gracefully.
    ...
    # --- Handle Video Sending/Editing ---
    if new_message_type == "video" and video_data:
        # 1. Prioritize Editing for a Smooth User Experience
        if not force_new_message and message_id_to_edit_or_delete:
            try:
                ...
                logger.warning(f"Editing message {message_id_to_edit_or_delete} failed: {e}. Will fall back to sending a new message.")
                sent_message = None # Explicitly mark editing as failed

        # 2. Handle Fallback: Send a new message if editing failed or was skipped
        if not sent_message:
            ...
            # Fallback A: Use the robust copy_message method first
            try:
                logger.info(f"Fallback: Attempting to send video {video_data['uuid']} via copy_message.")
                ...
                # This is a permanent failure - the video is gone from the channel.
                logger.error(f"copy_message failed permanently for {video_data['uuid']}: {e}. Deleting from DB.")
                ...
                logger.error(f"copy_message failed for {video_data['uuid']}: {e}. Trying final resort send_video.", exc_info=True)
                # Fallback B: Final resort using the original file_id
                try:
                    logger.info(f"Final Resort: Attempting to send video {video_data['uuid']} via send_video.")
                    ...
                    logger.info(f"Successfully sent video {video_data['uuid']} via send_video.")
                except FileReferenceExpired:
                    # This is an expected, temporary error. Do not delete the video.
                    logger.critical(f"FileReferenceExpired for {video_data['uuid']}. This is expected after a token change. NOT deleting video.")
                    return False, "A temporary error occurred. Please try the action again."
                except Exception as e2:
                    # If all methods have failed, the video is truly broken.
                    logger.error(f"Final resort send_video also failed for {video_data['uuid']}: {e2}. Deleting from DB.", exc_info=True)
