...
class BotConfig:
    ...
    MINI_APP_URL = "ab-tu-dek-beta-li3u.vercel.app" # IMPORTANT: Replace with your actual frontend URL
...
async def check_and_update_free_usage(user_id: int, is_batch_video: bool = False) -> bool:
    ...
    # If it's not a batch video, and the user is not premium/has no token, they can still watch.
    # This means regular (non-batch) videos are now free for users without tokens.
    if not is_batch_video:
        return True

    # The following logic only applies to batch videos for free users without tokens
    now = datetime.utcnow()
        ...
        if deep_link_arg:
            ...
            try:
                ...
                if deep_link_type == 'token_refresh':
                    ...
                    if not await check_and_update_free_usage(user_id, is_batch_video=True):
