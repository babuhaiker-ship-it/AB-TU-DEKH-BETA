...
sessions_collection = db['sessions'] # New collection for session strings
...
sessions_collection.create_index([("name", ASCENDING)], unique=True)

# --- Pyrogram MongoDB Session ---
class MongoSession(Storage):
    def __init__(self, name: str, collection):
        super().__init__(name)
        self._collection = collection
        self._session_data = self._fetch_session()

    def _fetch_session(self):
        doc = self._collection.find_one({'name': self.name})
        return doc['session_string'] if doc else None

    async def open(self):
        if self._session_data:
            self.session_string = self._session_data

    async def update(self):
        if self.session_string:
            self._collection.update_one(
                {'name': self.name},
                {'$set': {'session_string': self.session_string}},
                upsert=True
            )
    ...
    async def delete(self):
        self._collection.delete_one({'name': self.name})

# --- Pyrogram Client ---
# Initialize the client with the MongoSession storage
session_storage = MongoSession("spicynyraa_session", sessions_collection)
app = Client(
    name="spicynyraa",
    ...
    in_memory=True # Important: work in-memory and let MongoSession handle persistence
)
...
async def main():
    ...
    # After starting, if no session string was loaded, export and save it now.
    if not await app.storage.open():
        session_string = await app.export_session_string()
        await app.storage.update()
        logger.info("New session created and saved to MongoDB.")


    # Health check
    try:
        member = await app.get_chat_member(config.VIDEO_CHANNEL_ID, me.id)
        logger.info(f"Bot status in video channel: {member.status}")
        force_sub_member = await app.get_chat_member(config.FORCE_SUB_CHANNEL_ID, me.id)
        logger.info(f"Bot status in force subscribe channel: {force_sub_member.status}")
    except Exception as e:
