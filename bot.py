...
async def send_and_replace_message(client: Client, chat_id: int, message_id_to_edit_or_delete: int, new_message_type: str, video_data: dict = None, reply_markup: InlineKeyboardMarkup = None, text_content: str = None, force_new_message: bool = False) -> tuple[bool, Message | str]:
    \"\"\"
    Sends or edits a message with a robust, multi-step fallback system for videos.
    1. Prioritizes editing for a seamless experience.
    2. Handles FileReferenceExpired during edit as a non-fatal error, triggering a fallback.
    3. The primary fallback is to delete the old message and send a new one via copy_message.
    4. Only if all methods fail is the video considered broken and removed from the database.
    ...
    if new_message_type == \"text\":
        try:
            # If force_new_message, delete the old one first
            if force_new_message and message_id_to_edit_or_delete:
                try:
                    ...
                    logger.warning(f\"Could not delete old text message {message_id_to_edit_or_delete} before sending new: {e}\")
                message_id_to_edit_or_delete = None # Prevent trying to edit it

            # Try to edit first, if we can
        ...
        return False, \"Failed to send text message.\"


    # --- Handle Video Messages ---
    if new_message_type == \"video\" and video_data:
        # STEP 1: ATTEMPT TO EDIT THE MESSAGE
        if not force_new_message and message_id_to_edit_or_delete:
            try:
                ...
                logger.warning(f\"Editing message {message_id_to_edit_or_delete} failed with FileReferenceExpired: {fre}. This is expected after token changes. Proceeding to fallback (delete and send new).\")
                sent_message = None # Clear sent_message to trigger the fallback logic

            except Exception as e:
                logger.warning(f\"Editing message {message_id_to_edit_or_delete} failed with an unexpected error: {e}. Proceeding to fallback (delete and send new).\")
                sent_message = None # Clear sent_message to trigger the fallback logic

        # STEP 2: FALLBACK (if editing failed or was skipped by force_new_message)
        if not sent_message:
            # Fallback Part 1: Delete the old message if it exists
            if message_id_to_edit_or_delete:
                try:
                    await client.delete_messages(chat_id, message_id_to_edit_or_delete)
                    logger.info(f\"Deleted old message {message_id_to_edit_or_delete} in chat {chat_id} as part of fallback.\")
                except (MessageIdInvalid, UserIsBlocked, ChatInvalid):
                    logger.warning(f\"Old message {message_id_to_edit_or_delete} for fallback deletion was already invalid or inaccessible.\")
                except Exception as e:
                    logger.error(f\"Failed to delete old message {message_id_to_edit_or_delete} during fallback: {e}\")

            # Fallback Part 2: Try sending a new message using the most robust method (copy_message)
            try:
                logger.info(f\"Fallback: Attempting to send video {video_data['uuid']} via copy_message from channel.\")
                ...
                logger.info(f\"Fallback successful: Sent video {video_data['uuid']} via copy_message.\")
            
            except (MessageIdInvalid, ChatAdminRequired, ValueError) as e:
                # This is a strong indicator the video is truly gone or inaccessible. This is a FATAL error for the video.
                logger.error(f\"Fallback failed: copy_message failed for video {video_data['uuid']} (Msg ID: {video_data.get('message_id')}). Reason: {e}. The video is considered broken.\", exc_info=True)
                ...
                # If copy_message fails for another reason, it's a critical error.
                # Both edit and copy have failed. We consider it broken.
                logger.critical(f\"Fallback failed: Both edit and copy_message have failed for video {video_data['uuid']}. Error: {e}. The video is considered broken.\", exc_info=True)
                await client.send_message(chat_id, \"Oops! This video has become unavailable due to an unexpected error and has been removed. Trying the next one... 🔄\")
                await delete_broken_video_from_db_and_channel(client, video_data['uuid'], video_data.get('category'), video_data.get('sequence_number'), video_data.get('message_id'))
    ...
    if isinstance(sent_message, Message):
        ...
        # This path should now only be taken if there's a logic error or video_data was None.
        error_message = \"Failed to send or edit the message due to an unexpected error or missing video data.\"
