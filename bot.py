...
data_channel_collection.create_index([("_id", ASCENDING)], unique=True)
...
async def add_fsub_step1_callback(client: Client, callback_query: CallbackQuery):
    ...
    await callback_query.message.edit_text(
        "Please send the <b>Channel ID</b> (e.g., -1001234567890) of the force subscribe channel.\n\n"
...
async def set_data_channel_step1_callback(client: Client, callback_query: CallbackQuery):
    ...
    await callback_query.message.edit_text(
        "Please send the <b>Channel ID</b> (e.g., -1001234567890) for the video storage channel.\n\n"
    ...
    if is_admin(user_id):
        ...
        if user_id in admin_fsub_state and admin_fsub_state[user_id].get('step') == 'await_channel_id':
            try:
                ...
                if not (-1002000000000 < channel_id < -1001000000000): # Basic check for supergroup ID format
                    await message.reply("Invalid Channel ID format. Please send a valid supergroup ID (e.g., -1001234567890).")
                    return
                ...
                try:
                    ...
                    await message.reply("❌ Invalid Channel ID. Please check the ID and try again.")
                    return
                except ChatAdminRequired:
                    await message.reply("❌ Bot is not an admin in this channel or lacks necessary permissions (Invite Users). Please add the bot as admin and try again.")
                    return
                except Exception as e:
                    logger.error(f"Error getting chat info or invite link for channel {channel_id}: {e}")
                    await message.reply(f"An unexpected error occurred while getting channel info: {e}")
                    return
                ...
                await message.reply("Invalid Channel ID. Please send a valid integer ID.")
            except Exception as e:
                await message.reply(f"An error occurred: {e}")
        ...
        if user_id in admin_data_channel_state and admin_data_channel_state[user_id].get('step') == 'await_channel_id':
            try:
                ...
                try:
                    ...
                    logger.error(f"Error getting chat info for channel {channel_id}: {e}")
                    await message.reply(f"An unexpected error occurred while getting channel info: {e}")
                    return
