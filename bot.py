...
def video_nav_keyboard(
    ...
    # --- Like/Dislike Row ---
    buttons.append([
        InlineKeyboardButton("Dislike 👎", callback_data=f"dislike_next|{video_uuid}|{str_to_b64(category)}|{int(is_saved)}"),
        InlineKeyboardButton("Like 👍", callback_data=f"like_next|{video_uuid}|{str_to_b64(category)}|{int(is_saved)}")
    ])
...
@app.on_callback_query(filters.regex(r"^(like_next|dislike_next)\|(.+)\|(.+)\|(\d+)$"))
async def feedback_next_callback(client: Client, callback_query: CallbackQuery):
    """Handles the like/dislike button feedback and navigates to the next video."""
    await callback_query.answer("Thanks for your feedback ✅", show_alert=False)

    # Modify the callback data to match the navigate_video handler's expected format
    parts = callback_query.data.split('|')
    action, current_uuid, category_encoded, is_saved_flag_str = parts

    # Reconstruct the callback data for the navigate_video handler
    callback_query.data = f"next|{current_uuid}|{category_encoded}|{is_saved_flag_str}"

    # Call the navigate_video handler
