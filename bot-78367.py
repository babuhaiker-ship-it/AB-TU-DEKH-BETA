...
class BotConfig:
    ...
    # NEW: Set to 0 as per new plan, users get a free batch instead of initial tokens
    NEW_USER_TOKENS = 0
    REFERRAL_BONUS = 1
    REFRESH_BONUS = 1
    ...
    # --- New Feature Configuration (MODIFIED) ---
    # REMOVED: FREE_VIDEOS_LIMIT = 2
    # REMOVED: FREE_LIMIT_RESET_HOURS = 24
    FREE_BATCHES_LIMIT_PER_DAY = 1  # Number of free batches a user can watch per day
    FREE_BATCH_RESET_HOURS = 24  # Hours after which the free batch limit resets
...
# --- Free Video Limit Management (REWRITTEN) ---
# REMOVED: async def send_limit_reached_message(client: Client, chat_id: int): ... (This function is removed)

async def check_and_update_free_usage(user_id: int, is_new_batch_attempt: bool = False) -> bool:
    """
    Checks if a user can watch a video based on their token, premium, or free status.
    For free users, it manages the 'one full batch per day' logic.

    Args:
        user_id: The ID of the user.
        is_new_batch_attempt: True if the user is initiating a new browsing session
                              (e.g., clicking 'Get Video', selecting a category, opening Mini App).
                              False if they are navigating within an already started free batch.

    Returns:
        True if the user has access (premium, token, or consumed free batch).
        False if free batch limit is reached and no token/premium.
    """
    ...
    if not user_doc:
        ...
        users_collection.insert_one({
            'user_id': user_id,
            'free_usage': {'batches_consumed_today': 0, 'reset_at': now + timedelta(hours=config.FREE_BATCH_RESET_HOURS)}
        })
        user_doc = users_collection.find_one({'user_id': user_id})

    free_usage = user_doc.get('free_usage', {'batches_consumed_today': 0, 'reset_at': None})
    batches_consumed_today = free_usage.get('batches_consumed_today', 0)
    reset_at = free_usage.get('reset_at')

    # Reset daily limit if reset_at is in the past
    if not reset_at or now >= reset_at:
        batches_consumed_today = 0
        new_reset_at = now + timedelta(hours=config.FREE_BATCH_RESET_HOURS)
        users_collection.update_one(
            {'user_id': user_id},
            {'$set': {'free_usage': {'batches_consumed_today': 0, 'reset_at': new_reset_at}}},
            upsert=True
        )
        logger.info(f"Reset free batch limit for user {user_id}. Next reset at {new_reset_at}.")
        # Update local variable for current check
        free_usage['batches_consumed_today'] = 0
        free_usage['reset_at'] = new_reset_at

    # Handle new batch attempt
    if is_new_batch_attempt:
        if batches_consumed_today < config.FREE_BATCHES_LIMIT_PER_DAY:
            # Consume the free batch for today
            users_collection.update_one(
                {'user_id': user_id},
                {'$inc': {'free_usage.batches_consumed_today': 1}}
            )
            logger.info(f"User {user_id} consumed a free batch. Batches consumed: {batches_consumed_today + 1}/{config.FREE_BATCHES_LIMIT_PER_DAY}.")
            return True # Access granted for this new batch
        else:
            logger.warning(f"User {user_id} has reached their free batch limit.")
            return False # Free batch limit reached for today
    else:
        # Handle ongoing batch check (user is already within a free batch session)
        # If they've consumed at least one batch today, they can continue browsing within it.
        return batches_consumed_today > 0
...
# REMOVED: generate_token_earning_keyboard function is removed.
...
def get_premium_only_text() -> str:
    ...
    # NEW: Updated premium text
    return (
        "✨ <b>Become a VIP (Instant, Ad-Free Access)</b> ✨\n\n"
...
async def start_cmd(client: Client, message: Message):
    ...
    try:
        ...
        # --- Step 1: Handle new user registration and free usage initialization ---
        ...
        if is_new_user:
            ...
            users_collection.insert_one({
                ...
                # NEW: Initialize free_usage for batches
                'free_usage': {'batches_consumed_today': 0, 'reset_at': datetime.utcnow() + timedelta(hours=config.FREE_BATCH_RESET_HOURS)}
            })
            # REMOVED: add_token(user_id, config.NEW_USER_TOKENS * 86400, is_admin_granted=False)
            logger.info(f"New user registered: {user_id}. Initialized with free batch access.")
        ...
        if not has_joined:
            ...
            custom_text = None
            if is_new_user:
                # NEW: Updated new user welcome for force sub
                part1 = f"🎉 Congratulations {first_name_safe}!\n\nYour first full collection is unlocked for today! 🎁 Dive in and enjoy. 🔥\n\n"
                part2 = "To watch the content you requested, you just need to join our channels first. Once you've joined, tap the '🔄 Try Again' button below, and I'll take you straight to your content! 🚀" if deep_link_arg else "Please join our channels to continue. Once you've joined, tap the '🔄 Try Again' button below! 🚀"
                custom_text = part1 + part2
            else:
                custom_text = "Hey — quick thing before the next one:\nJoin our main & backup channels so you never lose access if anything goes wrong.\nIt’s just one tap. Don’t worry, we’ll keep the good stuff coming."
        ...
        if deep_link_arg:
            ...
            try:
                ...
                if deep_link_type == 'token_refresh':
                    ...
                    # NEW: Updated success message for token refresh
                    if success:
                        await message.reply("✅ Access Granted. Your 24-hour unlimited pass is now live.\nNothing will interrupt you. Enjoy the stream while it lasts.")
                    else:
                        await message.reply(msg) # Still show error message if failed
                    ...
                    # NEW: Updated access check for video share
                    if not await user_has_token(user_id) and not await check_and_update_free_usage(user_id, is_new_batch_attempt=True):
                        users_collection.update_one({'user_id': user_id}, {'$set': {'pending_command': deep_link_arg}})
                        await send_token_required_message(client, message, is_batch_consumed=True) # Use new message
                        return
                    ...
                    # NEW: Updated access check for batch deep link
                    if not await check_and_update_free_usage(user_id, is_new_batch_attempt=True):
                        await send_token_required_message(client, message, is_batch_consumed=True) # Use new message
                        return
            ...
            # NEW: Updated new user welcome message
            await message.reply(
                f"🎉 Congratulations {first_name_safe}!\n\n"
                f"Your first full collection is unlocked for today! 🎁 Dive in and enjoy. 🔥\n\n"
                f"Tap ‘🎞️ Get Video’ now and dive straight into your favorite category 🚀",
                reply_markup=await get_main_keyboard(user_id)
            )
        else: # Existing user, no deep link
            # NEW: Updated existing user welcome message
            await message.reply(f"👋 Welcome back, {first_name_safe}! 🌶️\n\nYour next free collection is ready. Tap '🎞️ Get Video' to dive in.", reply_markup=await get_main_keyboard(user_id))
...
async def get_video(client: Client, message: Message):
    ...
    # NEW: Updated access check for 'Get Video'
    if not await user_has_token(user_id) and not await check_and_update_free_usage(user_id, is_new_batch_attempt=True):
        await wait_msg.delete()
        logger.info(f"User {user_id} has no tokens or free batch, prompting earning options.")
        await send_token_required_message(client, message, is_batch_consumed=True) # Use new message
        return
    await wait_msg.delete()
...
async def select_category(client: Client, callback_query: CallbackQuery):
    ...
    # NEW: Access check for category selection
    if not await user_has_token(user_id) and not await check_and_update_free_usage(user_id, is_new_batch_attempt=False):
        await callback_query.answer("Your free batch is consumed or you need a token to continue.", show_alert=True)
        await send_token_required_message(client, callback_query.message, is_batch_consumed=True) # Use new message
        return
...
async def view_saved_category_callback(client: Client, callback_query: CallbackQuery):
    ...
    # NEW: Access check for saved category selection
    if not await user_has_token(user_id) and not await check_and_update_free_usage(user_id, is_new_batch_attempt=False):
        await callback_query.answer("Your free batch is consumed or you need a token to view saved videos.", show_alert=True)
        await send_token_required_message(client, callback_query.message, is_batch_consumed=True)
        return
...
async def navigate_video(client: Client, callback_query: CallbackQuery):
    ...
    try:
        ...
        try:
            ...
            # NEW: Access check for navigation
            if not await user_has_token(user_id) and not await check_and_update_free_usage(user_id, is_new_batch_attempt=False):
                await callback_query.answer("Your free batch is consumed or you need a token to continue.", show_alert=True)
                await send_token_required_message(client, callback_query.message, is_batch_consumed=True)
                return
...
async def navigate_batch_video(client: Client, callback_query: CallbackQuery):
    ...
    # NEW: Updated access check for batch navigation
    if not await check_and_update_free_usage(user_id, is_new_batch_attempt=False):
        await callback_query.answer("Your free batch is consumed or you need a token to continue.", show_alert=True)
        await send_token_required_message(client, callback_query.message, is_batch_consumed=True) # Use new message
        return
...
async def refer_btn(client: Client, message: Message):
    ...
    try:
        ...
        ref_link = f"https://t.me/{config.BOT_USERNAME[1:]}?start=ref_{user_id}"
        # NEW: Updated message text
        await message.reply(
            f"This is your referral link — share it, get tokens.\n"
            f"Invite friends, get free 24H passes. Simple.\n\n"
...
async def buy_token_btn(client: Client, message: Message):
    ...
    try:
        # NEW: Updated premium text and keyboard
        await message.reply(
            "✨ <b>Become a VIP (Instant, Ad-Free Access)</b> ✨\n\n"
            "✅ <b>Unlimited Saved Videos</b> — keep as many bookmarks as you like.\n"
            "✅ <b>Direct Video Downloads</b> — instantly get the actual file with no restrictions.\n"
...
async def refresh_token_btn(client: Client, message: Message):
    ...
    try:
        ...
        temp_msg = await message.reply("⏳ Please wait while we prepare your options... ✨")
        ...
        # NEW: Call the new send_token_required_message
        await send_token_required_message(client, message, is_batch_consumed=False) # Assuming this is a general prompt, not after a batch is consumed
        logger.info(f"User {user_id}: Refresh token message sent via send_token_required_message. Handler finished.")
    except Exception as e:
...
# REMOVED: send_token_earning_options function is removed.
...
async def saved_videos_btn(client: Client, message: Message):
    ...
    # NEW: Updated access check for saved videos
    if not await user_has_token(user_id) and not await check_and_update_free_usage(user_id, is_new_batch_attempt=True):
        logger.info(f"User {user_id} has no tokens or free batch, prompting earning options for saved videos.")
        await send_token_required_message(client, message, is_batch_consumed=True) # Use new message
        return
...
async def view_saved_video_callback(client: Client, callback_query: CallbackQuery):
    ...
    # NEW: Access check for viewing a specific saved video
    if not await user_has_token(user_id) and not await check_and_update_free_usage(user_id, is_new_batch_attempt=True):
        await callback_query.answer("You need a token or a free batch to watch this video. Please get access first! 🧐", show_alert=True)
        await send_token_required_message(client, callback_query.message, is_batch_consumed=True) # Use new message
        return
...
async def open_app_btn(client: Client, message: Message):
    ...
    # NEW: Updated access check for Mini App
    if not await user_has_token(user_id) and not await check_and_update_free_usage(user_id, is_new_batch_attempt=True):
        logger.info(f"User {user_id} has no tokens or free batch, cannot open Mini App.")
        await send_token_required_message(client, message, is_batch_consumed=True) # Use new message
        return
